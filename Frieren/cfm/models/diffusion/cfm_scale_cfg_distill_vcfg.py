...
class CFM_MAA2_CFG(DDPM):
    ...
    def shared_step(self, batch, train=False, **kwargs):
        x, c, noise = self.get_input(batch, self.first_stage_key, train=train)
        loss = self(x, c, noise=noise)
        return loss

    def forward(self, x, c, *args, **kwargs):
        # t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
        t = torch.zeros((x.shape[0],), device=self.device, dtype=torch.long)
        if self.model.conditioning_key is not None:
            assert c is not None
            if self.cond_stage_trainable:
                c = self.get_learned_conditioning(c)
                uncond_cond = self.get_learned_conditioning(self.zero_embed.to(self.device))
            if self.shorten_cond_schedule:  # TODO: drop this option
                tc = self.cond_ids[t].to(self.device)
                c = self.q_sample(x_start=c, t=tc, noise=torch.randn_like(c.float()))
        
        return self.p_losses(x, c, t, uncond_cond, *args, **kwargs)
    ...
    def p_losses(self, x_start, cond, t, uncond_cond, noise=None):
        ...
        t_in = torch.cat([t] * 2)
        x0_in = torch.cat([x0] * 2)
        cond_in = torch.cat([uncond_cond.expand(x0.shape[0], -1, -1), cond])
        
        # v_pred = self.apply_model(xt, t, cond)
        v_uncond, v_cond = self.apply_model(x0_in, t_in, cond_in).chunk(2)
        v_pred = v_uncond + 4.5 * (v_cond - v_uncond)

        x_pred = x0 + v_pred

        loss= torch.nn.functional.mse_loss(x1, x_pred,  reduction='none')
        loss_dict.update({f'{prefix}/loss': loss.mean().item()})
    ...
    def sample_log(self,cond,batch_size, ddim, ddim_steps, size_len=64, unconditional_guidance_scale=1.0,unconditional_conditioning=None, **kwargs):
        ...
        samples, intermediates = self.sample_param_cfg(cond=cond, batch_size=batch_size, timesteps=25,
    ...
    def log_sound(self, batch, N=4, n_row=4, sample=True, ddim_steps=250, ddim_eta=0.0, return_keys=None,
        ...
        log = dict()
        z, c, x, xrec, xc, noise = self.get_input(batch, self.first_stage_key,
        ...
        if sample:
            with self.ema_scope("Plotting"):
                ...
                samples, intermediates = self.sample_log(cond=c, batch_size=N, ddim=use_ddim, ddim_steps=ddim_steps, eta=ddim_eta, size_len=size_len, unconditional_guidance_scale=guidance_scale,unconditional_conditioning=uncond_cond, x_T=noise)
