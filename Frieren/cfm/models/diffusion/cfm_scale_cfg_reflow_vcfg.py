...
class CFM_MAA2_CFG(DDPM):
    ...
    def __init__(self,
        ...
        self.zero_embed = torch.zeros(1, video_cond_len, video_cond_dim, device=self.device)               # Zeros Embedding
    ...
    def p_losses(self, x_start, cond, t, uncond_cond, noise=None):
        ...
        t_in = torch.cat([t] * 2)
        xt_in = torch.cat([xt] * 2)
        cond_in = torch.cat([uncond_cond.expand(xt.shape[0], -1, -1), cond])
        
        # v_pred = self.apply_model(xt, t, cond)
        v_uncond, v_cond = self.apply_model(xt_in, t_in, cond_in).chunk(2)
