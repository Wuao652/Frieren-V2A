...
from cfm.modules.new_attention import PositionEmbedding
...
from cfm.modules.new_attention import CrossAttention,Conv1dFeedForward,checkpoint,Normalize,zero_module
...
class ChannelConcatDiT(nn.Module):
    """
    ...
    def __init__(
        ...
        self.c_upsampler = nn.Upsample(scale_factor=8, mode='nearest')
        ...
        self.x_proj = nn.Linear(hidden_size, hidden_size // 2)
        self.c_proj = nn.Linear(hidden_size, hidden_size // 2)
    ...
    def forward(self, x, t, context):
        ...
        c = rearrange(c, 'b t c -> b c t')
        c = self.c_upsampler(c)
        x = self.proj_in(x)

        x = rearrange(x, 'b c t -> b t c')
        c = rearrange(c, 'b c t -> b t c')

        x = self.x_proj(x)
        c = self.c_proj(c)

        xc = torch.concat([x, c], dim=2)
        xc = torch.concat([t, xc], dim=1)
        xc = self.pos_emb(xc)
        xc = rearrange(xc,'b t c -> b c t')
        for block in self.blocks:
            xc = block(xc)                      # (N, D, extra_len+T)

        xc = xc[..., 1:]
        xc = self.final_layer(xc)                # (N, out_channels,T)
        return xc
